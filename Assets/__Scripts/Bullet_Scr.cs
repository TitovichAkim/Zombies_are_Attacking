...
public class Bullet_Scr : MonoBehaviour {
    ...
    public float        damage;                     // Наносимый урон
    ...
    void FixedUpdate() {
        ...
        if(Time.time - createMoment > 5) {          // Если разница между временем создания и текущим моментом больше 5 секунд
    ...
    public void OnTriggerEnter2D(Collider2D collision) {                                            // Если прикоснулся к чему-то
        if (collision.gameObject.layer != 6 & collision.gameObject.name != "Visibility_Area") {     // Если это не игрок и не его зона обзора
            Destroy(this.gameObject);                                                               // Самоуничтожиться
            }
        }
    }
